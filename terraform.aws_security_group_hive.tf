resource "aws_security_group" "hive_server" {
  name        = "Hive Server Security Group"
  description = "Hive Server Security Group"  # see https://www.stefaanlippens.net/hadoop-3-default-ports.html
  ingress {
    description = "ssh"
    protocol = "tcp"
    from_port = 22
  ...
  ingress {
    description = "hive.server2.thrift.port"
    protocol = "tcp"
    from_port = 10000
    to_port = 10000
    cidr_blocks = ["0.0.0.0/0"]
  ...
  ingress {
    description = "hive.server2.webui.host"
    protocol = "tcp"
    from_port = 10002
    to_port = 10002
    cidr_blocks = ["0.0.0.0/0"]
  ...
  ingress {
    description = "Hive Web UI - hive.hwi.listen.port"
    protocol = "tcp"
    from_port = 9999
    to_port = 9999
    cidr_blocks = ["0.0.0.0/0"]
  ...
  ingress {
    description = "Hive Metastore - hive.metastore.uris"
    protocol = "tcp"
    from_port = 9083
    to_port = 9083
    cidr_blocks = ["0.0.0.0/0"]
  ...
  ingress {
    description = "dfs.hivenode.address"
    protocol = "tcp"
    from_port = 9866
    to_port = 9866
    cidr_blocks = ["0.0.0.0/0"]
  ...
  ingress {
    description = "dfs.hivenode.http.address"
    protocol = "tcp"
    from_port = 9864
    to_port = 9864
    cidr_blocks = ["0.0.0.0/0"]
  ...
  ingress {
    description = "dfs.hivenode.ipc.address"
    protocol = "tcp"
    from_port = 9867
    to_port = 9867
    cidr_blocks = ["0.0.0.0/0"]
  ...
  tags = {
    Name = "HDFS Hivenode Security Group"
  }
}